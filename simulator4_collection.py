...
while True:
    ...
    for i in hs300_data.index:
        ...
        if flag and len(in_stock_dict) < max_stock:
            ...
            if len(candidates) == 0:
                continue
            candidates = pd.DataFrame(candidates, columns=['stock_code'])
            candidates['rank'] = candidates['stock_code'].apply(
                lambda x: collection[collection['code'] == int(x)].index.values[0] if int(x) in collection[
                    'code'].values else np.random.randint(100,999))
            targets=candidates.sort_values(by='rank').head(min(len(candidates), max_stock - len(in_stock_dict)))
    ...
    records = pd.DataFrame(records, columns=['code', 'buy_date', 'sell_date', 'days', 'buy_price', 'sell_price', 'rate',
                                             'money'])
    print('hs300 collection', 'hs300_close > hs300_ma30 and hs300_ma_v_30 > hs300_ma_v_120', 'close/pre_close<=1.05', 'max_stock=',
          max_stock,
          start_date,
          end_date,
