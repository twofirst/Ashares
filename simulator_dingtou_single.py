...
while True:
    ...
    for hs_idx in hs300_data.index:
        ...
        if hs_idx % invest_term_days == 0 and close < avg_price:
            ...
            rcd = {'name': current_stock_name, 'invest_times': invest_times,
                   'total_share': total_share, 'value': total_share * 100 * sell_price,
                   'total_invest': total_invest, 'buy_date': buy_date, 'sell_date': sell_date}
            in_stock_dict[current_stock_code] = rcd
            current_stock_code = None
            # print('sell', trade_date, sell_price, sell_share, value, total_share, total_invest, cash, weight)
            # print(code, total_money, cash / total_money, rcd)
    else:
        profit = 0
