...
file = './Files/stock_period_hs300_20090101-20181231_close > ma_30 and not bar_stop_not flag and close <= ma_30.csv'
start_date = '20090101'
end_date = '20181231'
...
for stock_code in data.keys():
    ...
    for i in hs300_data.index:
        ...
        if flag and not in_stock_flag:
            ...
            vol_30 = stock_data.ix[stock_index - 1, 'ma_v_30']
            vol_120 = stock_data.ix[stock_index - 1, 'ma_v_120']
            ...
            if pre_close < pre_ma20 and close > ma_20 and not bar_stop and change > 0 and vol > pre_vol and vol > vol_120 and vol_30 > vol_120 and close / ma_20 <= 1.05:
                # if close > ma_30 and not bar_stop:
            ...
            # elif not flag and in_stock_flag:
            if trade_date not in stock_data['trade_date'].values:
                continue
            stock_index = stock_data[stock_data['trade_date'] == trade_date].index.values[0]
            close = stock_data.ix[stock_index, 'close']
...
df = pd.DataFrame(df, columns=['code', 'trade_times', 'rate_min', 'rate_mean', 'rate_max', 'success_rate', 'end_rate'])
